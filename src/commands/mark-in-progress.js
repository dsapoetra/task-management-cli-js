...
export const runMarkInProgressTask = (returnToMainMenu) => {
  inquirer
    .prompt([
      {
        type: "input",
        name: "taskName",
        message: "What's the task to be mark in progess?",
      }
    ])
    .then((answers) => {
      set(answers.id, { name: answers.taskName, status: "in progress" });
